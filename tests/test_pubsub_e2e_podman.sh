...
CONTAINER_ID=$(podman run -d -p $REDIS_PORT:6379 redis:alpine)

# Ensure container is destroyed when script exits
trap "echo ''; print_info 'Cleaning up...'; podman rm -f $CONTAINER_ID > /dev/null 2>&1" EXIT
...
if podman exec $CONTAINER_ID redis-cli ping > /dev/null 2>&1; then
