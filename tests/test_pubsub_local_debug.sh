...
################################################################################
# Redis Pub/Sub End-to-End Test Script (Local Redis)
# 
...
#
# PREREQUISITES:
# - Redis server running locally on port 6379 (or specify custom port)
# - Go 1.19+ installed
################################################################################
...
if command -v redis-cli &> /dev/null; then
    if redis-cli -h $REDIS_HOST -p $REDIS_PORT ping > /dev/null 2>&1; then
        ...
        echo "Or specify custom host/port: REDIS_HOST=host REDIS_PORT=port $0"
        exit 1
    fi
else
...
if [ -n "$SUB_ID_BUFFER" ]; then
    REQUEST='{"jsonrpc":"2.0","id":43,"method":"pubsub_unsubscribe","params":{"params":{"driver":"redis","host":"'$REDIS_HOST'","port":'$REDIS_PORT',"database":"'$REDIS_DB'"},"subscription_id":"'$SUB_ID_BUFFER'"}}'
    send_request "$REQUEST" > /dev/null
fi

if [ -n "$SUB_ID_QUERY" ]; then
    REQUEST='{"jsonrpc":"2.0","id":44,"method":"pubsub_unsubscribe","params":{"params":{"driver":"redis","host":"'$REDIS_HOST'","port":'$REDIS_PORT',"database":"'$REDIS_DB'"},"subscription_id":"'$SUB_ID_QUERY'"}}'
    send_request "$REQUEST" > /dev/null
fi

print_success "Cleaned up all test subscriptions"
